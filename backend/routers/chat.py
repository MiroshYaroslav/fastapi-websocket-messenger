...
async def websocket_endpoint(
    ...
    try:
        query = select(Message).where(Message.room_id == room_id).order_by(Message.timestamp.desc()).limit(10)
        ...
        while True:
            ...
            if text:
                ...
                is_recipient_in_room = (
                        ...
                        recipient_id in manager.active_connections.get(room_id, {})
                )
...
async def get_chat_history(
        room_id: int,
        offset: int = Query(0, description="Скільки повідомлень пропустити"),
        limit: int = Query(10, description="Скільки повідомлень завантажити"),
        db: AsyncSession = Depends(get_db),
...
async def get_unread_counts(
    ...
    query = select(Message.sender_id, func.count(Message.id)) \
        .where(
...
async def mark_messages_as_read(
    ...
    room_id = min(current_user.id, sender_id) * 100000 + max(current_user.id, sender_id)
